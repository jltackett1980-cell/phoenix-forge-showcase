#!/usr/bin/env python3
"""
Build A Shelter Management - Backend API
Domain: custom_build_a_shelter | Entities: Patient
...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'custom_build_a_shelter-secret-v2'
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'custom_build_a_shelter.db')
...
def index():
    ...
    if os.path.exists(p):
        return send_file(p)
    return jsonify({"app": "Build A Shelter Management", "status": "running"})

@app.route("/api/health", methods=["GET"])
def health():
    return jsonify({"status": "healthy", "app": "Build A Shelter Management", "domain": "custom_build_a_shelter", "timestamp": datetime.now().isoformat()})
...
def stats():
    try:
        ...
        return jsonify({"total": total, "domain": "custom_build_a_shelter"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
