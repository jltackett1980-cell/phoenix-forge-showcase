#!/usr/bin/env python3
"""
LegalDesk - Backend API
Domain: law | Entities: Case, Client
...
def index():
    ...
    if os.path.exists(p):
        return send_file(p)
    return jsonify({"app": "LegalDesk", "status": "running"})

@app.route("/api/health", methods=["GET"])
def health():
    return jsonify({"status": "healthy", "app": "LegalDesk", "domain": "law", "timestamp": datetime.now().isoformat()})
...
def get_cases():
    try:
        ...
        with get_db() as db:
            if search:
                rows = db.execute("SELECT * FROM cases WHERE name LIKE ? OR status LIKE ? ORDER BY created_at DESC", (f"%{search}%", f"%{search}%")).fetchall()
            else:
...
def create_case():
    try:
        ...
        with get_db() as db:
            db.execute("INSERT INTO cases (case_number, client_name, attorney, case_type, filing_date, court_date, opposing_counsel, status, retainer_paid, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [data.get(f, "") for f in ['case_number', 'client_name', 'attorney', 'case_type', 'filing_date', 'court_date', 'opposing_counsel', 'status', 'retainer_paid', 'notes']])
            db.commit()
        log_activity(request.user.get("user"), "create_case")
        return jsonify({"message": "Case created"})
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def update_case(item_id):
    try:
        data = request.get_json()
        with get_db() as db:
            db.execute(f"UPDATE cases SET case_number=?, client_name=?, attorney=?, case_type=?, filing_date=?, court_date=?, opposing_counsel=?, status=?, retainer_paid=?, notes=?, updated_at=? WHERE id=?", [data.get(f, "") for f in ['case_number', 'client_name', 'attorney', 'case_type', 'filing_date', 'court_date', 'opposing_counsel', 'status', 'retainer_paid', 'notes']] + [datetime.now().isoformat(), item_id])
            db.commit()
        log_activity(request.user.get("user"), "update_case", item_id)
...
def create_client():
    try:
        ...
        if not data:
            return jsonify({"error": "No data provided"}), 400
        with get_db() as db:
            db.execute("INSERT INTO clients (case_id, name) VALUES (?, ?)", [data.get(f, "") for f in ['case_id', 'name']])
            db.commit()
...
def update_client(item_id):
    try:
        data = request.get_json()
        with get_db() as db:
            db.execute(f"UPDATE clients SET case_id=?, name=?, updated_at=? WHERE id=?", [data.get(f, "") for f in ['case_id', 'name']] + [datetime.now().isoformat(), item_id])
            db.commit()
...
@app.route("/api/cases/<int:parent_id>/clients", methods=["GET"])
@token_required
def get_case_clients(parent_id):
    try:
        with get_db() as db:
            rows = db.execute("SELECT * FROM clients WHERE case_id=? ORDER BY created_at DESC", (parent_id,)).fetchall()
        return jsonify([dict(r) for r in rows])
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/cases/<int:parent_id>/clients", methods=["POST"])
@token_required
def create_case_client(parent_id):
    try:
        data = request.get_json()
        if not data:
            return jsonify({"error": "No data provided"}), 400
        with get_db() as db:
            db.execute("INSERT INTO clients (case_id, name) VALUES (?, ?)", [parent_id] + [data.get(f, "") for f in ['case_id', 'name'][1:]])
            db.commit()
        return jsonify({"message": "client created"})
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
# ── STATUS ──
@app.route("/api/cases/<int:item_id>/status", methods=["PATCH"])
@token_required
def update_case_status(item_id):
    try:
        data = request.get_json()
        ...
        with get_db() as db:
            db.execute("UPDATE cases SET status=?, updated_at=? WHERE id=?", (new_status, datetime.now().isoformat(), item_id))
            db.commit()
...
def stats():
    try:
        ...
        return jsonify({"total": total, "domain": "law"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
